...
flags.DEFINE_string(
    "input_file", 'output/test.tfrecord',
...
def run_epoch( epoch, sess, evaluate, eval_op, input_ids, lm_losses, saver, root_path, save_step, mid_save_step, phase, batch_size=16, train_op=tf.constant(0)):
    ...
    try:
        while True:
            ...
            n_sample = len(y)
            n_all += n_sample
            # total_lm_acc += n_sample * masked_lm_accuracy
            # total_mean_loss += n_sample * masked_lm_mean_loss
            # total_next_acc += n_sample * next_sentence_accuracy
            # total_next_loss += n_sample * next_sentence_mean_loss
...
def parse_exmp(serial_exmp):
    ...
    return ids, msk, sids, m_lp, m_lids, m_lm_w, nsl
...
def main(_):
    ...
    input_ids, input_mask, segment_ids, masked_lm_positions, masked_lm_ids, masked_lm_weights,next_sentence_labels = iterator.get_next()
    features = [input_ids, input_mask, segment_ids, masked_lm_positions, masked_lm_ids, masked_lm_weights,
                next_sentence_labels]
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(FLAGS.num_train_epochs):
            ...
            run_epoch(epoch, sess, evaluate, eval_op, input_ids, loss, saver, root_path, save_step, FLAGS.mid_save_step,'train', batch_size=train_batch_size, train_op=train_op)
