...
with open('data/HAE_REBED_dic.pkltrain', 'rb') as f:
    train_dic = pkl.load(f)
with open('data/HAE_REBED_dic.pklvalid', 'rb') as f:
    val_dic = pkl.load(f)
with open('data/DSTC8_answer_OriginalID2NewID.pkl','rb') as f:
    ans_dic = pkl.load(f)
...
def worker(input_name):
    ...
    train_examples = create_examples(
        read_DSTC( os.path.join(input_dir, "Advising_dstc_train_{}.txt".format(str(input_name)))),
    ...
    write_instance_to_example_files(train_features, [ os.path.join(output_dir,'Advising_REBED_dstc_train_{}.tfrecord'.format(str(input_name)))])
...
if __name__ == "__main__":
    ...
    print("***** Read dev_data *****")
    dev_examples = create_examples(read_DSTC(os.path.join(input_dir, "Advising_dstc_valid.txt")), "dev")         # [exp1, exp2...] example X is a class object: {guid, text_a(str), text_b(str), label(str)}
    dev_features = convert_examples_to_features(dev_examples, label_list, max_seq_length, tokenizer, val_dic)
    write_instance_to_example_files(dev_features, [ os.path.join(output_dir,'Advising_dstc_valid.tfrecord')])
