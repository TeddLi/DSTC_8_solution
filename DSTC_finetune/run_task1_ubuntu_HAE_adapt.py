...
flags.DEFINE_string(
    "adapt_model_dir", 'adapted_L-12_H-768_A-12/bert_final',
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, HAE_ids, labels,
  ...
  with tf.variable_scope("loss"):
    ...
    debug_package = [ logits, probabilities, loss, labels]

    losses = tf.multiply(loss, all_target_loss)
    ...
    return mean_loss, debug_package, logits, probabilities, accuracy, model
...
def run_epoch(epoch_no, op_name, sess , training, logits, accuracy, mean_loss, debug_package, train_opt=tf.constant(0)):
    n_updates = 0
    t_loss = 0
    n_all = 0
    t0 = time()
    try:
        while True:
            n_updates += 1
            batch_logits, batch_loss, _ , accur,debug_package_= sess.run([logits, mean_loss, train_opt, accuracy,debug_package],feed_dict={training:True})
            n_sample = batch_logits.shape[0]
            logits_, probabilities_, loss_, labels_ = debug_package_
            n_all += n_sample
...
def main(_):
    ...
    set = list(range(FLAGS.num_train_epochs))
    ...
    # sample_num = total_sample(FLAGS.train_dir + str(980))
    sample_num = total_sample(FLAGS.train_dir + str(set[0]) + '.tfrecord')
    ...
    mean_loss, debug_package, logits, probabilities, accuracy, model= create_model(bert_config,
    ...
    train_opt = optimization.create_optimizer(mean_loss, learning_rate, num_train_steps, num_warmup_steps,
                                              False)
    ...
    adapt_model_dir = FLAGS.adapt_model_dir
    # valid_file = 'DSTC8_DATA/Task_1/ubuntu/data/DSTC_valid_hae.tfrecordtttt'

    train_set = random.sample(set, FLAGS.num_train_epochs)
    ...
    if FLAGS.do_train:
        with tf.Session(config=config) as sess:
            ...
            ckpt = tf.train.get_checkpoint_state(adapt_model_dir)  # '/media/E/xiaoyu/repos/nli_bert/output/mrpc_output')
            if ckpt:
                variables = tf.trainable_variables()
            ...
            for epoch in range(FLAGS.num_train_epochs):
                # train_file_use = train_file + str(train_set[epoch]) + '.tfrecord'
                train_file_use = train_file + str(0) + '.tfrecord'
                tf.logging.info('Epoch {} Training begin, picked set {}'.format(epoch, train_set[epoch]))

                sess.run(iterator.initializer,
                       feed_dict={filenames: [train_file_use], shuffle_size: 1024})
                run_epoch(epoch, "train", sess, training, logits, accuracy, mean_loss, debug_package, train_opt)
